    ...
    public partial class Role : Page, INotifyPropertyChanged
    {
        ...
        private async Task DeleteSelectedRoleAsync()
        {
            if (lst.SelectedItem != null)
            {
                ...
                {
                    MessageBox.Show("Роль 'Admin' нельзя удалить.");
                    return;
                }

                MessageBoxResult result = MessageBox.Show("Вы уверены, что хотите удалить эту роль?", "Подтверждение удаления", MessageBoxButton.YesNo);
